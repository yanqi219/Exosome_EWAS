...
ClockMatrix <- 
  ...
  dplyr::select("Age", "AgeAccelerationResidual", "AgeAccelerationResidualHannum", "DNAmAgeSkinBloodClockAdjAge", "DNAmTLAdjAge", "AgeAccelPheno", "AgeAccelGrim", "DNAmADMAdjAge", "DNAmB2MAdjAge", "DNAmCystatinCAdjAge", "DNAmGDF15AdjAge", "DNAmLeptinAdjAge", "DNAmPACKYRSAdjAge", "DNAmPAI1AdjAge", "DNAmTIMP1AdjAge", "CD8.naive", "CD4.naive", "CD8pCD28nCD45RAn", "PlasmaBlast", "CD4T", "NK", "Mono", "Gran")
colnames(ClockMatrix) <- c("Age", "AgeAccelHorvath", "AgeAccelHannum", "AgeAccelSkinBlood", "AgeAccelTL", "AgeAccelPheno", "AgeAccelGrim", "DNAmADMAdjAge", "DNAmB2MAdjAge", "DNAmCystatinCAdjAge", "DNAmGDF15AdjAge", "DNAmLeptinAdjAge", "DNAmPACKYRSAdjAge", "DNAmPAI1AdjAge", "DNAmTIMP1AdjAge", "CD8.naive", "CD4.naive", "CD8pCD28nCD45RAn", "PlasmaBlast", "CD4T", "NK", "Mono", "Gran")
...
png(file="/Users/qiyan/Dropbox/Horvath_Lab/Onging_Project/Exosome_EWAS_Clement_local/Results/Figures/DNAmClock_corr.png", width=2800, height=2800)
ggheatmap + 
...
paired_t <- function(input_matrix = sample_matrix, var){
  ...
  bxp <- ggpaired(input_matrix, x = "Time", y = var, id = "BH.ID", color = "Time", line.color = "gray", line.size = 2.5, point.size = 7, palette = "jco", order = c("Before", "After"), ylab = var, xlab = "Time", legend = "none") +
  ...
  bxp <- ggpar(bxp, font.tickslab = c(23,"bold"), font.x = c(23,"bold"), font.y = c(23,"bold"))  + 
    geom_text(data = Means, aes(x = factor(Time), y = Avg, label = paste("Mean = ",round(Avg, 2), sep = "")), nudge_x = .4, size = 9, fontface = "bold")
  # Add p-value and significance levels
  temp_plot <- bxp + stat_compare_means(method = "t.test", paired = T, ref.group = "Before", size = 9, fontface = "bold.italic")
...
# Paired barplots
png(file="/Users/qiyan/Dropbox/Horvath_Lab/Onging_Project/Exosome_EWAS_Clement_local/Results/Figures/AIs/CellType_boxplot.png", width=2933, height=1600)
...
ggarrange(CD8.naive[2][[1]], CD4.naive[2][[1]], CD8pCD28nCD45RAn[2][[1]], PlasmaBlast[2][[1]], CD4T[2][[1]], NK[2][[1]], Mono[2][[1]], Gran[2][[1]], labels = NA, ncol = 4, nrow = 2, vjust = 1,font.label = list(size = 14, face = "bold"))
dev.off()
```
...
final_result <- rbind(statistics_DNAmAge, statistics_GrimAge, statistics_celltype, statistics_biomarker) %>%
  ...
  dplyr::filter(!(Variable %in% c("Cytomegalovirus (CMV) Ab IgM", "Immature Grans (Abs)", "Immature Granulocytes", "Lipoprotein (a)")))
...
```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
statistics_rescale <- statistics %>%
  dplyr::filter(!(clock %in% c("Cytomegalovirus (CMV) Ab IgM", "Immature Grans (Abs)", "Immature Granulocytes", "Lipoprotein (a)"))) %>%
...
paired_t <- function(input_matrix = sample_matrix, var){
  ...
  Means <- input_matrix %>% group_by(Time) %>% 
    summarize(Avg = mean(.data[[var]]))
  bxp <- ggpaired(input_matrix, x = "Time", y = var, id = "BH.ID", color = "Time", line.color = "gray", line.size = 2, point.size = 7, palette = "jco", order = c("Before", "After"), ylab = var, xlab = "Time", legend = "none") +
    geom_point(data = Means, mapping = aes(x = Time, y = Avg), color = "red", shape = 18, cex = 9) +
    geom_line(data = Means, mapping = aes(x = Time, y = Avg, group = 1), size = 2.5, linetype = 2, color = "red") +
    theme(plot.margin = unit(c(4,4,4,4), "lines")) +
    coord_cartesian(clip = "off")
...
Similarly, we also used Fisher’s test to assess enrichment of significant DMRs in
